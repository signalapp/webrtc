...
typedef struct {
  ...
  void (*onIceNetworkRouteChange)(void* observer_borrowed,
                                  ...
                                  const char* remote_description_borrowed);

  // Media events
  void (*onAddStream)(void* observer_borrowed,
                      webrtc::MediaStreamInterface* stream_owned_rc);
  void (*onAddAudioRtpReceiver)(
      void* observer_borrowed,
      webrtc::MediaStreamTrackInterface* track_owned_rc);
  void (*onAddVideoRtpReceiver)(
      void* observer_borrowed,
      webrtc::MediaStreamTrackInterface* track_owned_rc,
      uint32_t demux_id);
  void (*onVideoFrame)(void* observer_borrowed,
                       uint32_t track_id,
  ...
  bool (*encryptMedia)(void* observer_borrowed,
                       const uint8_t* plaintext_borrowed,
                       size_t,
                       uint8_t* ciphertext_out,
                       size_t,
                       size_t* ciphertext_size_out);
  size_t (*getMediaPlaintextBufferSize)(void* observer_borrowed,
                                        uint32_t,
                                        bool,
                                        size_t);
  bool (*decryptMedia)(void* observer_borrowed,
                       uint32_t,
                       const uint8_t* ciphertext_borrowed,
                       size_t,
...
RUSTEXPORT void Rust_deletePeerConnectionObserver(
    webrtc::rffi::PeerConnectionObserverRffi* observer_owned);
