...
class VoiceMediaShimChannel : public VoiceMediaChannel {
 public:
  ...
  void ConfigureEncoders(const webrtc::AudioEncoder::Config& config) override {
    return send_impl()->ConfigureEncoders(config);
  }

  // RingRTC change to get audio levels
  void GetCapturedAudioLevel(cricket::AudioLevel* captured_out) override {
    return send_impl()->GetCapturedAudioLevel(captured_out);
  }
  ...
  void GetReceivedAudioLevels(
    ...
    size_t* received_size_out) override {
      return receive_impl()->GetReceivedAudioLevels(
        received_out, received_out_size, received_size_out);
  }
